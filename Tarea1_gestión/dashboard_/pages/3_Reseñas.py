...
with c1:
    st.markdown(kpi_card("Longitud media", f"{avg_review_len} palabras", "Promedio por reseña"), unsafe_allow_html=True)
with c2:
    st.markdown(kpi_card("Rating medio reseñas", avg_review_rating, "Promedio de usuarios"), unsafe_allow_html=True)
with c3:
    st.markdown(
        kpi_card(
...
with right:
    ...
    fig = px.box(
        ...
        labels={"genre": "Género", "review_length_words": "Palabras por reseña"}
    )
...
with bottom_right:
    ...
    fig = px.bar(
        ...
        labels={"rating_gap": "Diferencia absoluta", "title_short": "Película"}
    )
