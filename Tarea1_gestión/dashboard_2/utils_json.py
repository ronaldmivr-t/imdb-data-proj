...
def make_binned_bar(
    ...
    """
    Crea un gráfico de barras a partir de una variable continua agrupada en rangos.
    Esta versión usa una sola traza para que las barras no salgan delgadas.
    """
    temp = df.copy()

    temp[f"{column}_bin"] = pd.cut(
        temp[column],
        bins=bins,
        labels=labels,
        include_lowest=True,
    ...
    summary = (
        temp.groupby(f"{column}_bin", observed=False)
        .size()
    ...
    fig = px.bar(
        ...
        title=title,
        labels={
            f"{column}_bin": x_label,
            "count": "Número de registros"
        }
    )

    # Colores manuales por barra, sin separar en múltiples trazas
    if color_sequence is not None:
        fig.update_traces(marker_color=color_sequence[:len(summary)])
...
def make_binned_bar_by_group(
    ...
    """
    Crea barras agrupadas por una categoría adicional.
    Se ajusta el espacio para que el grosor se vea consistente.
    """
    temp = df.copy()
    ...
    summary = (
        temp.groupby([f"{column}_bin", group_col], observed=False)
        .size()
        .reset_index(name="count")
    )
